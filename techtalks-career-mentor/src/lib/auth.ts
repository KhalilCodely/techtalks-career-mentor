import jwt from "jsonwebtoken";
import { cookies } from "next/headers";

type JWTPayload = {
  userId: string;
  role: "USER" | "ADMIN";
};

export type AuthenticatedUser = {
  userId: string;
  role: "USER" | "ADMIN";
};

/**
 * Resolves the current user from middleware headers (if present), else JWT from
 * Authorization Bearer or the `token` cookie. For use in Route Handlers (Node).
 */
export async function getAuthenticatedUser(req: Request): Promise<AuthenticatedUser> {
  const middlewareUserId = req.headers.get("x-user-id");
  const middlewareRole = req.headers.get("x-user-role");

  if (
    middlewareUserId &&
    (middlewareRole === "USER" || middlewareRole === "ADMIN")
  ) {
    return {
      userId: middlewareUserId,
      role: middlewareRole,
    };
  }

  const authHeader = req.headers.get("authorization");
  const bearerToken = authHeader?.replace(/^Bearer\s+/i, "").trim() ?? "";
  const cookieStore = await cookies();
  const cookieToken = cookieStore.get("token")?.value ?? "";
  const token = bearerToken || cookieToken;

  if (!token) {
    throw new Error("Unauthorized");
  }

  if (!process.env.JWT_SECRET) {
    throw new Error("Server auth configuration is missing");
  }

  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET) as JWTPayload;
    return { userId: decoded.userId, role: decoded.role };
  } catch {
    throw new Error("Invalid or expired token");
  }
}

export function requireAdmin(req: Request) {
  const middlewareRole = req.headers.get("x-user-role");
  const middlewareUserId = req.headers.get("x-user-id");

  if (middlewareRole === "ADMIN" && middlewareUserId) {
    return {
      userId: middlewareUserId,
      role: "ADMIN" as const,
    };
  }

  const authHeader = req.headers.get("authorization");

  if (!authHeader) {
    throw new Error("No authorization header");
  }

  const token = authHeader.replace(/^Bearer\s+/i, "").trim();

  if (!token) {
    throw new Error("No token provided");
  }

  if (!process.env.JWT_SECRET) {
    throw new Error("Server auth configuration is missing");
  }

  const decoded = jwt.verify(token, process.env.JWT_SECRET) as JWTPayload;

  if (decoded.role !== "ADMIN") {
    throw new Error("Forbidden - Admins only");
  }

  return decoded;
}